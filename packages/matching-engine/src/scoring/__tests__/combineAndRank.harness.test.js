 ...
 *
 * @param {import('@locvm/types').LocumJob} job
 * @param {import('@locvm/types').Physician[]} physicians
 * @returns {ScoredPair[]}
 */
...
describe('Harness: combineAndRank end-to-end', () => {
  ...
  it('all breakdown values are in [0, 1]', () => {
    for (const r of allResults) {
      for (const key of Object.keys(r.breakdown) /** @type {keyof import('@locvm/types').ScoreBreakdown} */) {
        const val = r.breakdown[/** @type {keyof import('@locvm/types').ScoreBreakdown} */ (key)]
  ...
  it('no NaN values in scores or breakdown', () => {
    for (const r of allResults) {
      ...
      for (const key of Object.keys(r.breakdown) /** @type {keyof import('@locvm/types').ScoreBreakdown} */) {
        expect(Number.isNaN(r.breakdown[/** @type {keyof import('@locvm/types').ScoreBreakdown} */ (key)])).toBe(false)
      }
    }
  })
