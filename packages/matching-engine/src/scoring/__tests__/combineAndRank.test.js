...
describe('score bounds', () => {
  ...
  it('breakdown values are always in [0, 1]', () => {
    ...
    for (const key of Object.keys(breakdown)) {
      expect(breakdown[/** @type {keyof import('@locvm/types').ScoreBreakdown} */ (key)]).toBeGreaterThanOrEqual(0)
      expect(breakdown[/** @type {keyof import('@locvm/types').ScoreBreakdown} */ (key)]).toBeLessThanOrEqual(1)
    }
  })
})
