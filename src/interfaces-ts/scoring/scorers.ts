// LOCVM Matching Engine - Scoring Function Contracts
//
...
import type { Physician, GeoCoordinates, ProvinceCode } from "../core/models";
 ...
 *
 * @param physician - the clean physician profile
 * @param jobLocation - the jobs geographic coordinates
 * @returns score between 0 and 1 where 1 = same spot, 0 = super far
 */
export type ScoreLocationFn = (
  physician: Physician,
  jobLocation: GeoCoordinates
) => number;
 ...
 *
 * @param physician - the clean physician profile
 * @param jobDateRange - the jobs start and end dates
 * @returns score between 0 and 1 where 1 = full date-range coverage
 */
export type ScoreDurationFn = (
  physician: Physician,
  jobDateRange: { from: Date; to: Date }
) => number;
 ...
 *
 * @param physicianEMRs - the physicians known EMR systems
 * @param jobEMR - the EMR system used at the jobs facility (may be undefined)
 * @returns score between 0 and 1
 */
export type ScoreEMRFn = (
  physicianEMRs: string[],
  jobEMR: string | undefined
) => number;
 ...
 *
 * @param physician - the clean physician profile
 * @param jobProvince - the 2-letter province code where the job is located
 * @returns score between 0 and 1
 */
export type ScoreProvinceFn = (
  physician: Physician,
  jobProvince: ProvinceCode | undefined
) => number;
 ...
 *
 * @param physician - the clean physician profile
 * @param jobSpeciality - the required speciality for the job
 * @returns score between 0 and 1
 */
export type ScoreSpecialityFn = (
  physician: Physician,
