// LOCVM Matching Engine - Filter Function Contracts
//
// Hard filters and checks that decide if a physician even qualifies before any scoring happens

import type { Physician, LocumJob } from "../core/models";
import type { matchingCriteria } from "./matching";
 ...
 * @param criteria - the search criteria (contains required profession, speciality, etc)
 * @returns true if the physician should be scored, false if excluded
 */
export type IsEligiblePhysicianFn = (
  physician: Physician,
 ...
 *
 * @param job - the clean locum job
 * @returns true if the job should trigger an immediate short-term matching run
 */
export type IsShortTermJobFn = (job: LocumJob) => boolean;
