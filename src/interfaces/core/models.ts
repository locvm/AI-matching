...
//
// Everything gets normalized before it touches the matching engine
// These models = the canonical shapes all modules work with
...
/**
 * Normalized address. Province is always a 2-letter code by the time it gets here (normalization layer handles the messy stuff)
 */
export type Address = {
  streetNumber?: string;
  streetName?: string;
  city?: string;
  /** always normalized to 2-letter code like "ON", "AB", "BC" */
 ...
 *
 * All province data MUST be normalized to one of these before it
 ...
 *   "British Columbia" -> "BC"
 *
 * The normalization logic (fuzzy matching, trimming) lives in code.
...
/**
 * Normalized physician for the matching engine.
 *
 ...
 *   - 180 doctors have isLookingForLocums completely missing (not true, not false, just gone). We treat missing as true. Max's call, signing up implies interest
 *   - 177 out of 410 doctors have 0 location info. So location is nullable and scoring handles it as neutral
 *   - Only 42 out of 410 have emrSystems filled in. Optional, neutral when missing
 *   - preferredProvinces has all the messy variants. Normalized to ProvinceCode[]
 */
export type Physician = {
   ...
   *
   * Missing values -> true during normalization.
   * Max's decision: "physicians signing up to LOCVM are doing so because they are interested in locum opportunities."
   *
   * Eve confirmed the field wasnt required at first and some poeple just never filled it in. She made it required like 3-4 months in
   * So the missing ones are likely interested, just never got to that step
   */
  isLookingForLocums: boolean;

  /**
   * Doctor's anchor location (from workAddress or geocoded from city/province).
   *
   * Null for 43% of physicians. When null, location scoring returns a neutral score, doesnt penalize, doesnt boost
   * Eve's current algo uses lat/lng distance scoring for those who have it
   */
  location: GeoCoordinates | null;

  /** Normalized work address */
  ...
  /** Provinces the doctor prefers to work in. Normalized to codes. */
  preferredProvinces: ProvinceCode[];

  /**
   * EMR systems the doctor knows.
   * Only 42 out of 410 have this. When empty, EMR scoring = neutral
   * Eve said its a nice feature but not a dealbreaker, she's not even calculating it in her current algo.
   */
  emrSystems: string[];
   ...
   * DB stores these as month/year strings ("november" / "2025"), the normalization layer converts to proper Dates
   */
  ...
  /** Whether the doctor's profile is complete */
 ...
 *
 * The DB stores these as month/year strings which is not great for date math haha. Normalization layer converts them to real Dates
 */
 ...
 * Until then, EMR scoring treats missing job EMR as neutral
 */
export type LocumJob = {
  ...
  /** Experience level, free text, pretty messy. Needs future normalization` */
  experience?: string;

  /** Pay amount as string like "8000". Future: normalize to number */
  locumPay?: string;
 ...
 * We need this to check for scheduling conflicts, dont want to match someone who's already booked for overlapping dates
 */
