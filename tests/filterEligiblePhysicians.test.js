...
describe("filterEligiblePhysicians", () => {
  it("missing preferences object: physician still passes (isLookingForLocums missing → treated as true)", () => {
    const physicians = [{ ...basePhysician, _id: "a", preferences: undefined }];
  ...
  it("missing isLookingForLocums: physician still passes (only lenient field)", () => {
    const physicians = [{ ...basePhysician, _id: "a", preferences: {} }];
    const result = filterEligiblePhysicians(physicians, baseJob, undefined, {});
    expect(result).toHaveLength(1);
  });
  ...
  it("physician profession mismatch: excluded", () => {
    const physicians = [{ ...basePhysician, medProfession: "Recruiter" }];
    const result = filterEligiblePhysicians(physicians, baseJob, undefined, {});
    expect(result).toHaveLength(0);
  });
  ...
  it("reservation excludes physician via applicants (physician id in reservation.applicants)", () => {
    ...
    const result = filterEligiblePhysicians(
      physicians,
      baseJob,
      reservation,
      {},
  ...
  it("reservation excludes physician via applicant stages (multiple applicants)", () => {
    ...
    const reservation = {
      applicants: [{ userId: "user1" }, { userId: "user3" }],
    };
    const result = filterEligiblePhysicians(
      physicians,
      baseJob,
...
describe("filterEligiblePhysicians – duration filter", () => {
  ...
  it('short job: physician with "a few days" (1–7d) passes (overlaps short bucket)', () => {
    const physicians = [
      { ...basePhysician, locumDurations: [{ minDays: 1, maxDays: 7 }] },
    ];
    const result = filterEligiblePhysicians(
      physicians,
      shortJob,
  ...
  it('short job: physician with "less than a month" (1–30d) passes', () => {
    const physicians = [
      { ...basePhysician, locumDurations: [{ minDays: 1, maxDays: 30 }] },
    ];
    const result = filterEligiblePhysicians(
      physicians,
      shortJob,
  ...
  it('short job: physician with "1–3 months" (30–90d) passes (overlaps short bucket)', () => {
    const physicians = [
      { ...basePhysician, locumDurations: [{ minDays: 30, maxDays: 90 }] },
    ];
    const result = filterEligiblePhysicians(
      physicians,
      shortJob,
  ...
  it('short job: physician with only "3–6 months" (90–180d) passes (overlaps short bucket at 90)', () => {
    const physicians = [
      { ...basePhysician, locumDurations: [{ minDays: 90, maxDays: 180 }] },
    ];
    const result = filterEligiblePhysicians(
      physicians,
      shortJob,
  ...
  it('short job: physician with only "6+ months" (180–365d) excluded', () => {
    const physicians = [
      { ...basePhysician, locumDurations: [{ minDays: 180, maxDays: 365 }] },
    ];
    const result = filterEligiblePhysicians(
      physicians,
      shortJob,
...
describe("filterEligiblePhysicians – province filter", () => {
  ...
  it("province match via multiple preferredProvinces: passes when any matches", () => {
    const physicians = [
      { ...basePhysician, preferredProvinces: ["BC", "ON", "AB"] },
    ];
    const result = filterEligiblePhysicians(physicians, jobON, undefined, {});
    expect(result).toHaveLength(1);
  });
