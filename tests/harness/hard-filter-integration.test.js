...
function runHardFilterWithSampling(sampling = {}) {
  ...
  for (const job of sampledJobs) {
    const reservation = fixtures.reservations.find((r) => r.locumJobId === job._id) ?? null
    const criteria = {
      job,
    ...
    for (const { job, eligible } of results) {
      ...
      for (const p of eligible) {
        ...
        const hasOverlap = durations.some(
          (/** @type {{ minDays: number, maxDays: number }} */ d) => d.minDays <= bucket.max && d.maxDays >= bucket.min
        )
