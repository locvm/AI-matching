...
describe("Hard filter – harness-backed integration", () => {
  ...
  it("no returned physician has a duration mismatch with the job", () => {
    ...
    for (const { job, eligible } of results) {
      ...
      const jobDays =
        (new Date(job.dateRange.to).getTime() -
          new Date(job.dateRange.from).getTime()) /
        MS_PER_DAY;
      ...
      for (const p of eligible) {
        ...
        const hasOverlap = durations.some(
          (/** @type {{ minDays: number, maxDays: number }} */ d) =>
...
describe("Hard filter – configurable sampling", () => {
  it("runs with custom maxJobs, maxUsers, seed", () => {
    const { sampledJobs, sampledPhysicians, results } =
      runHardFilterWithSampling({
        maxJobs: 10,
        maxUsers: 500,
        seed: 123,
      });
